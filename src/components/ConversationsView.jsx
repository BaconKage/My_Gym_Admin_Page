  ...
  useEffect(() => {
    async function load() {
      try {
        setLoading(true);
        setError("");
        const res = await fetchCollectionData("conversations", 1, 50);
        setData(res);
      } catch (err) {
        console.error("Failed to load conversations", err);
        setError("Failed to load conversations from MongoDB.");
      } finally {
        setLoading(false);
      }
    }
    ...
    <div className="container mx-auto px-4 py-8 space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground text-sm">
          Live view of documents from the <code>conversations</code>{" "}
          collection.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription className="text-xs">
            Total documents in <code>conversations</code>:{" "}
